...
class DeepMultiscaleSpeckleNet(nn.Module):
    def __init__(self, outdim):
        ...
        # Final fully connected layers
        self.fc = nn.Sequential(
            nn.Linear(4096, 1024),  # 入力次元を16に変更
            # nn.LeakyReLU(0.2),
            nn.PReLU(),
            nn.Dropout(0.1),
            nn.Linear(1024, 4096),  # 入力次元を16に変更
            # nn.LeakyReLU(0.2),
            nn.PReLU(),
            nn.Dropout(0.1),
...
class NewDeepMultiscaleSpeckleNet(nn.Module):
    def __init__(self, outdim):
        super(NewDeepMultiscaleSpeckleNet, self).__init__()
        ...
        # グローバルプーリングを追加
        self.global_pool = nn.AdaptiveMaxPool1d(512)  # 出力サイズを1に固定

        # Final fully connected layers
        self.fc = nn.Sequential(
            nn.Linear(512, 512),  # 入力次元を16に変更
            # nn.LeakyReLU(0.2),
            nn.PReLU(),
            nn.Dropout(0.1),
            nn.Linear(512, 512),  # 入力次元を16に変更
